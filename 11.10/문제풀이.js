...
movies.sort((a,b) => {
    ...
    const disA = Math.sqrt(Math.pow(a.action - target.action, 2) + Math.pow(a.kiss - target, 2) )
    const disB = Math.sqrt(Math.pow(b.action - target.action, 2) + Math.pow(b.kiss - target, 2) )
